...
/// Multuply a matrix in CSR format with a dense vector. The vector is on the right hand side of the
/// matrix.
...
template <typename T, typename = std::enable_if_t<std::is_same_v<std::remove_volatile_t<T>, float>>>
...
/// Multuply a matrix in CSR format with a dense vector and subtract this from a vector. Performing
/// lhs - A * mult
/// @param[in] rows The number of rows of the matrix
/// @param[in] rowStart Array with length the number of rows + 1,
/// holding where each row starts in columnIndex and values arrays
...
/// @param[in] mult The vector which multiples the matrix (should not overlap with res)
/// @param[out] res The result of the vector matrix product (should not overlap with rhs, can
/// overlap with lhs)
...
template <typename T, typename = std::enable_if_t<std::is_same_v<std::remove_volatile_t<T>, float>>>
__device__ void saxpy(const int vectorLength, const float a, const T* x, T* y) {
...
template <typename T, typename = std::enable_if_t<std::is_same_v<std::remove_volatile_t<T>, float>>>
__device__ void dotProduct(const int vectorLength, const T* a, const T* b, T* result) {
	...
	if (cacheIndex == 0) {
		atomicAdd((std::remove_volatile_t<T>*)result, cache[0]);
	}
}
...
namespace Sync {
	namespace {
		...
		__device__ volatile unsigned int barrier = 0;
	}  // namespace
	class GridSync {
	public:
#if HAS_COOP_GROUPS
		...
		void __device__ sync() {
			...
			if (threadIdx.x == 0) {
				...
				const unsigned int oldCount =
					atomicInc(const_cast<unsigned*>(&barrier), gridDim.x - 1);
				if (oldCount == gridDim.x - 1) {
					atomicAdd(const_cast<unsigned*>(&generation), 1);
				}
				while (atomicCAS(const_cast<unsigned*>(&generation), myGeneration, myGeneration) ==
					   myGeneration)
					;
			}
			__syncthreads();
...
extern "C" __global__ void conjugateGradientMegakernel(CGParams<!HAS_COOP_GROUPS> params) {
	const int maxIterations = params.maxIterations;
	const int tid = blockIdx.x * blockDim.x + threadIdx.x;
	const int rows = params.rows;
	Sync::GridSync gridSync;
